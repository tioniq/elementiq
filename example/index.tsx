import { createVar, type Vary } from "@tioniq/eventiq"
import {
  Button,
  ...
  render,
  ThemeContext,
} from "@tioniq/elementiq"

function AnotherView() {
  return <span>Hello, you tapped:</span>
}

function View() {
  const theme = createVar("dark")
  const count = createVar(0)
  const variants: Array<keyof Button.Variant> = [
    "normal",
    "primary",
    "secondary",
    "success",
  ...
  const appearances: Array<keyof Button.Appearance> = [
    "normal",
    "solid",
    "outline",
    "link",
        ...
        <Button
          onClick={() => {
            ...
            dynamicElements.value = [
              dynamicElements.value,
              <Button>Dynamic {random.toString()}</Button>,
            ]
